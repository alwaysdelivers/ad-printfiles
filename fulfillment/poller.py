...
def line_opts(node):
    ...
    return opts.get("color"), opts.get("size"), opts.get("print style") or opts.get("style") or opts.get("version") or opts.get("colorway")
...
def build_items(node):
    ...
    for e in node["lineItems"]["edges"]:
        ...
        ink = next((a["value"] for a in attrs if (a.get("key") or "").strip().lower() == "ink"), None)
        prefix = next((a["value"] for a in attrs if (a.get("key") or "").strip().lower() == "prefix"), None)
        price = (li.get("originalUnitPriceSet") or {}).get("shopMoney", {}).get("amount")
        item = fulfill.line_to_item(li["title"], color, size, li["quantity"], price, pstyle, ink, prefix)
        if item.get("error"):
